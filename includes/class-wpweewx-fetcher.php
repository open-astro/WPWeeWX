...
class WPWeeWX_Fetcher {
	...
	public static function fetch_live( $source ) {
		...
		if ( null === $decoded || ! is_array( $decoded ) ) {
			return array(
				'error'       => __( 'Invalid JSON response.', 'wpweewx' ),
				'http_status' => $code,
			);
		}
	...
	public static function test_fetch( $source ) {
		...
		if ( isset( $result['data'] ) && is_array( $result['data'] ) ) {
			...
			if ( isset( $result['data']['current'] ) && is_array( $result['data']['current'] ) ) {
				$metrics = array_keys( $result['data']['current'] );
			}

			if ( isset( $result['data']['generation'] ) ) {
